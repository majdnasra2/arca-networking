...
//     let dst = vec![0u8; CHUNK_SIZE];
//     let mut total_read = 0u64;

//     // change uint32_t transfer_started = 1 in the shared memory
    
//     println!("Starting reading...");
    
//     // while (total_read < transfer_size)

//         // aval_len = atomic read(end_index) - atomic read(start_index)
        
//         // if (aval_len > 0)
//             // barrier (the read of end_index, start_index should happen before any memcpy)
            
//             // len = min (CHUNK_SIZE, aval_len)

//             // read_start = start_idx % shm_size
//             // l = min(len, shm_size - read_start);

//             // memcpy(dst, read_start, l);
// 	        // memcpy(dst + l, data_start, len - l);
        
//             // barrier (the write of end_index should happen after all memcpy)  smp_wmb()
            
//             // atomic write read_start += len
//             // total_read += len
//         // else
//             // spins and waits
    
//     // change uint32_t transfer_started = 0 in the shared memory
// }
